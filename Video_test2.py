...
def validate_license_plate(strFinalString):
  """
  This function validates a string to ensure it matches the format of a license plate:
  - 8 or 9 characters long
  - Starts with 2 digits
  - Followed by 1 letter
  - Ends with 5 or 6 digits
  """
  pattern = r"^\d{2}[A-Z]\d{5,6}$"
  if re.match(pattern, strFinalString):
    return True
  else:
    return False
...
cap = cv2.VideoCapture(0) # Use the primary camera (webcam)
    ...
    if detected == 1:
        ...
        for screenCnt in screenCnt:
            ...
            if len(char_x) in range(7, 10):
                ...
                if (validate_license_plate(strFinalString)):
                    print("\n License Plate  is: " + first_line + " - " + second_line + "\n")
                    print("Openingg barrier")
